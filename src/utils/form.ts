import { createWeek, updateWeek, updateMatchups } from '../resources/espn';
import {
  CreateWeekInput,
  WeekFormValues,
  FormMatchup,
...
export const submitUpdateWeek = async (
  weekFormValues: WeekUpdateFormValues,
  weekId: number
) => {
  const week = {
    ID: weekId,
...
export const weekFormToDb = (
  weekFormValues: WeekFormValues
): CreateWeekInput => {
  let weekInput = {
...
export const createMatchupFormToDb = (
  matchupFormValues: FormMatchup[],
  weekId: number
): CreateMatchupsInput[] => {
  const matchups = matchupFormValues
    .filter((matchup) => matchup?.home?.ID != 0 && matchup?.away?.ID != 0)
    .map((matchup) => ({
      WeekID: weekId,
...
export const updateMatchupFormToDb = (
  matchupFormValues: FormUpdateMatchup[]
): MatchupsInput[] => {
  const matchups = matchupFormValues
    .filter((matchup) => matchup?.home?.ID != 0 && matchup?.away?.ID != 0)
    .map((matchup) => ({
